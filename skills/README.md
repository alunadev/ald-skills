...
- **Triggers**: "clarify", "enrich this prompt", "help me describe this better", CLARIFIER\_ADVISORY in context, any prompt under 10 words with no file path or error message, "fix the bug", "add authentication", "make this better", "refactor this", "clean this up", "improve performance", "add payments", "build the feature", "make it work".
...
### 50. Delegate to Hermes

- **Path**: `skills/delegate-to-hermes/`
- **Purpose**: Isolated Claude-to-Hermes CLI handoff for a bounded coding task, with a self-contained prompt file, a separate worktree/branch, and Claude's diff review and independent checks before acceptance. Includes a live validation gate because the CLI orchestration pattern has not yet been proved end to end.
